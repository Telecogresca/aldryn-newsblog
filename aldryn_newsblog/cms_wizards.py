...
class CreateNewsBlogArticleForm(BaseFormMixin, TranslatableModelForm):
    ...
    content_article = forms.CharField(
        label=_('Content'),
        required=False,
    ...
    class Meta:
        model = Article
        fields = ['title', 'app_config', "content_article"]
    ...
    def save(self, commit=True):
        ...
        # If 'content_article' field has value, create a TextPlugin with same and add
        # it to the PlaceholderField
        content_article = clean_html(self.cleaned_data.get('content_article', ''), False)
        if content_article and permissions.has_plugin_permission(
